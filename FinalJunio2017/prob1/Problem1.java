package FinalTest.FinalJunio2017.prob1;
...
public class Problem1 {
	...
	public static List<String> elementsInBoth(List<String> list1, List<String> list2) {
		//implement
		return list1.stream().filter(l1 -> list2.contains(l1)).collect(Collectors.toList());
	}
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		//implement
		return list.stream().filter(c -> c.getCity().length() > 5 && !c.getCity().contains("e"))
				.map(c -> c.getZip())
				.sorted()
				.distinct()
