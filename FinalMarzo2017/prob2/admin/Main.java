package FinalTest.FinalMarzo2017.prob2.admin;
import java.time.LocalDate;
import java.util.*;

import FinalTest.FinalMarzo2017.prob2.Course;
import FinalTest.FinalMarzo2017.prob2.PlanOfStudy;
import FinalTest.FinalMarzo2017.prob2.Student;
import FinalTest.FinalMarzo2017.prob2.Transcript;
...
public class Main {
	...
	public static List<Course> coursesTaughtByValentine(List<Student> allStudents) {
		...
		for (Student s:allStudents) {
			for (Course c:s.getPlanOfStudy().getCourses()) {
				if ((c.getPrimaryProfessor()=="Valentine")&&(!coursesFound.contains(c)))
					coursesFound.add(c);
			}
		}
		return coursesFound;
