package FinalTest.FinalMarzo2017.prob1;
import java.util.*;
...
public class Problem1 {
	...
	public static List<Customer> specialCustomers(List<Account> accts) {
		return accts.stream().filter(x -> x.getId().length()==x.getCustomer().getAddress().getCity().length())
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		return custs.stream().filter(c -> c.getCheckingAccount().getBalance() > 50 && c.getCheckingAccount().getBalance() < 100)
				.filter(c -> c.getFirstName().startsWith("A")).sorted(Comparator.comparing(Customer::getLastName)).collect(Collectors.toList());
		//implement          
	}
