...
public class Main {
	...
	private void showAllOrderItems() {
		System.out.println("\n==============\nAll order items:");
		//implement
		//Stream<String> listItems = orders.stream().flatMap(o -> Stream.of(o.getOrderItems().toString())).map(s -> characterStream(s));
		//Stream<String> listItems = orders.stream().flatMap((o -> o.getOrderItems()).stream().flatMap(oi -> oi.toString()));
		//Stream<String> listItems = orders.stream().flatMap(o -> Stream.of(o.getOrderItems())).forEach(System.out::println);;
		List<OrderItem> listItems = orders.stream() 
